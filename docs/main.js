  ...
  if (cityName.value === '' || countryCode.value === '') { // If fields are not filled show warning
    ...
    try { // try section, when code below contains an error, we catching this error
      ...
      if (xhr.status != 200 && xhr.readyState != 4) { // if response is not OK, handle an error
        ...
        const _wTemp = Math.round(wData.main.temp - 276); // converting temperature to Celsius, because in response temperature is in Fahrenheits
        ...
        if (wList.has(wData.name)) {
          ...
          document.querySelector(`#${wData.name}_weatherIcon`).innerHTML = '<img src=' + '"' + 'https://openweathermap.org/img/wn/' + wData.weather[0]['icon'] + '@2x.png' + '"' + ' alt="weatherIcon" ' + '/>'; // refreshing the weather image
        } else {
          ...
          childSource.style.display = 'block'; // showing the block
          childSource.innerHTML = `<div id='${wData.name}' class='name'>${wData.name}</div>` + // adding name information to name block
            `<div id='${wData.name}_description' class='description'>${wData.weather[0]['description']}</div>` + // adding description information to description block
            `<div id='${wData.name}_weatherDeg' class='weatherDeg'>${Math.round(wData.main.temp - 276) + '&deg;'}</div>` + // adding temperature information to temperature block and add html deg entity
            `<div id='${wData.name}_weatherIcon' class='weatherIcon'>${'<img src=' + '"' + 'https://openweathermap.org/img/wn/' + wData.weather[0]['icon'] + '@2x.png' + '"' + ' alt="weatherIcon" ' + '/>'}</div>`; // add weather image url to image block
